...
// Schema check: seller_id must exist on clothes, otherwise the seller join below will fail
$schema_check = $connect->query("SHOW COLUMNS FROM clothes LIKE 'seller_id'");
if (!$schema_check || $schema_check->num_rows === 0) {
    die("Database error: the <code>clothes</code> table is missing the <code>seller_id</code> column. Please run <a href='alter_database.php'>alter_database.php</a> once, then reload this page.");
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_product'])) {
    ...
    if (empty($errors)) {
        ...
        $insert_stmt = $connect->prepare("INSERT INTO clothes (product_name, category, subcategory, price, on_sale, sale_price, stock_quantity, description, image_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $insert_stmt->bind_param("sssdidiss", $product_name, $category, $subcategory, $price, $on_sale, $sale_price_val, $stock_quantity, $description, $image_path);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_product'])) {
    ...
    // Look up the current image so it's preserved if no new file is uploaded
    $current_image = null;
    $existing_stmt = $connect->prepare("SELECT image_url FROM clothes WHERE product_id = ?");
    $existing_stmt->bind_param("i", $product_id);
    ...
    if (empty($errors)) {
        ...
        $update_stmt = $connect->prepare("UPDATE clothes SET product_name = ?, category = ?, subcategory = ?, price = ?, on_sale = ?, sale_price = ?, stock_quantity = ?, description = ?, image_url = ? WHERE product_id = ?");
        $update_stmt->bind_param("sssdidissi", $product_name, $category, $subcategory, $price, $on_sale, $sale_price_val, $stock_quantity, $description, $image_path, $product_id);
...
if (isset($_GET['delete_id'])) {
    ...
    // Look up the image first so we can remove it from disk too
    $img_stmt = $connect->prepare("SELECT image_url FROM clothes WHERE product_id = ?");
    $img_stmt->bind_param("i", $delete_id);
    $img_stmt->execute();
    $img_row = $img_stmt->get_result()->fetch_assoc();
    $img_stmt->close();

    $delete_stmt = $connect->prepare("DELETE FROM clothes WHERE product_id = ?");
...
// Fetch item for editing (if edit clicked)
$edit_product = null;
if (isset($_GET['edit_id'])) {
    $edit_id = intval($_GET['edit_id']);
    $edit_query = $connect->prepare("SELECT * FROM clothes WHERE product_id = ?");
...
// Fetch all clothing items, along with seller username if applicable
$products_query = "SELECT c.*, u.username AS seller_username FROM clothes c LEFT JOIN users u ON c.seller_id = u.user_id ORDER BY c.product_id DESC";
$products_result = $connect->query($products_query);

if ($products_result === false) {
    // Likely cause: the clothes table is missing the seller_id column.
    // Run alter_database.php once to add it, then reload this page.
    die("Database error: " . $connect->error . "<br><br>It looks like the <code>clothes</code> table may be missing the <code>seller_id</code> column. Please run <a href='alter_database.php'>alter_database.php</a> once, then return to this page.");
}
?>
...
<head>
    ...
    <title>Manage Clothing - Admin Dashboard</title>
    <style>
        * {
            margin: 0;
            padding: 0;
    ...
    <div class="admin-container">
        <div class="admin-header">
            ...
            <div class="header-actions">
                <a href="admin_dashboard.php" class="nav-btn">← Customers</a>
        ...
        <div class="modal">
            <div class="modal-content">
                <span class="close-modal" onclick="window.location.href='admin_clothes.php'">&times;</span>
            ...
            <div class="table-responsive">
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td colspan="8" style="text-align:center; color:#999; padding: 30px;">No clothing items yet. Add one above to get started.</td>
                        </tr>
                        <?php endif; ?>
                        ...
                        <tr>
                            ...
                            <td><?php echo $p['seller_username'] ? htmlspecialchars($p['seller_username']) : '<span style="color:#999;">Admin/Store</span>'; ?></td>
                            <td>
